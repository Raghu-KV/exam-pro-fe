...
import { useLazyGetStudentCompletedTestsQuery } from "../redux/requests/studentRequest";
  ...
  return (
    <div className="font-inter mb-16 px-4">
      <p className=" font-bold text-center text-3xl mt-4">
        Completed Tests by the student
      </p>
      {/* SEARCH COMPO */}
      <FilterCompo
      ...
      {data?.length ? (
        <div className="grid grid-cols-1 gap-3 mt-6 lg:gap-6 lg:grid-cols-3">
          {data?.map((item) => (
            <SingleCard data={item} key={item._id} isButton={true} />
          ))}
        </div>
      ) : (
...
export default StudentCompleteTestCompo;
